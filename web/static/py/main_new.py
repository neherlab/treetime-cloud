...
def _update_session_state(root, state, err=None, warn=None):
    ...
    _write_json(state, os.path.join(root, "session_state.json"))
    print (state)
...
def process(root, config_dic):
    ...
    err=None    
    if config_dic["do_build_tree"] == True:
        _update_session_state(root, state, err=err) 
        try:
            build_tree(root)
        except:
            err = "Error in TreeTime run: cannot build phylogenetic tree."
            _update_session_state(root, err)
            return
    ...
    try:
        ...
        _update_session_state(root, state, err)
        return   
    ...
    try:
        ...
        _update_session_state(root, state, warn=warn)
