...
class TreeTimeWeb(treetime.TreeTime):
  ...
  def run(self, **kwargs):
    ...
    do_marginal = (False if self._webconfig["do_marginal"] == "False" or
                   not self._webconfig["do_marginal"] else True)
    resolve_polytomies = (False if self._webconfig["polytomies"] == "False" or
                          not self._webconfig["polytomies"] else True)
    slope = (None if self._webconfig["slope"] == "False" or
             not self._webconfig["slope"] else float(
                 self._webconfig["slope_value"]))
    ...
    try:
      ...
      for n in self.tree.find_clades():
        ...
        if len(n.mutations):
          n.comment = (
              '&mutations="' +
              ",".join([a + str(pos) + d for (a, pos, d) in n.mutations]) + '"')
    except:
      tb = traceback.format_exc()
      _write_session_state(
          self._root_dir,
  ...
  def _node_metadata(self, node):
    ...
    relax_clock = (False if self._webconfig["use_relaxed_clock"] == "False" or
                   not self._webconfig["use_relaxed_clock"] else True)
    if relax_clock:
  ...
  def _tips_data_to_json(self):
    ...
    arr = [{
        ...
        "strain":
            k.name,
        "numdate_given":
            k.numdate_given if hasattr(k, "numdate_given") else 0.0,
        "numdate":
            k.numdate if hasattr(k, "numdate") else 0.0,
        "xValue":
            k.xvalue if hasattr(k, "xvalue") else 0.0,
